    ...
    public Task<DataSnapshot> getValue(final Query query) {
        ...
        scheduleNow(new Runnable() {
            public void run() {
                ...
                if (persisted.exists()) {
                    Repo.this.scheduleDelayed(new Repo$9$$ExternalSyntheticLambda1(source, persisted), 3000);
                }
                Repo.this.connection.get(query.getPath().asList(), query.getSpec().getParams().getWireProtocolParams()).addOnCompleteListener((Executor) ((DefaultRunLoop) Repo.this.ctx.getRunLoop()).getExecutorService(), new Repo$9$$ExternalSyntheticLambda0(this, source, persisted, query));
            }

            /* access modifiers changed from: package-private */
            /* renamed from: lambda$run$1$com-google-firebase-database-core-Repo$9  reason: not valid java name */
            public /* synthetic */ void m39lambda$run$1$comgooglefirebasedatabasecoreRepo$9(TaskCompletionSource source, DataSnapshot persisted, Query query, Task task) {
                if (!source.getTask().isComplete()) {
                    if (task.isSuccessful()) {
                        Node serverNode = NodeUtilities.NodeFromJSON(task.getResult());
                        Repo repo = Repo.this;
                        repo.postEvents(repo.serverSyncTree.applyServerOverwrite(query.getPath(), serverNode));
